        ...
        function onRequest(scriptContext) {
            ...
            if (scriptContext.request.method == 'GET') {
                try {
                    ...
                    log.debug("get_approval_users", "TranId="+txn0.tranId+", txn0.internalId="+txn0.internalId);
                    ...
                    if (!typeOfApprovalIsHOD && txn.properties.employeeFieldName.length > 0
                        ...
                        if (txn.typeDependentEmployeeId) {
                            ...
                            if (approvalMatrixMainRow.mailToEmployeeHod) {
                            ...
                            else { // ************* check supervisor email & limit **************
								...
								if (txnEmployeesSupervisor && txnEmployeesSupervisor.supervisor.length > 0){
									var supervisor_rec = record.load({ type: "employee", id: txnEmployeesSupervisor.supervisor[0].value, isDynamic: true });
									supervisorPurchaseLimit = parseFloat(supervisor_rec.getValue("purchaseorderapprovallimit")) || 0;
									supervisorEmail = supervisor_rec.getValue("email");
									log.debug("get_approval_users::GET", "supervisorPurchaseLimit=" + supervisorPurchaseLimit + ", txn.total=" + txn.total);
									if (parseFloat(txn.total) > parseFloat(supervisorPurchaseLimit)) {
										log.debug("get_approval_users::GET", "supervisor Limit check failed");
									}
									else if(!supervisorEmail){
										log.debug("get_approval_users::GET", "supervisor - email is empty");
									}
									else{
                    ...
                    if (employeeRoleCategoryFilter.length > 0) {
                        ...
                        employeeSearchObj2.run().each(function (result) {
							...
							var emp_email = result.getValue({ name: "email", join: "email" });
                            //if(exchange_rate==0){
							...
							// limit was not available in User's subsidiary base currency in a saved search in any shape or form.
							...
							employeeEmail = emp_rec.getValue("email"); 
							// this could come from saved search, but there is another 
							// check for the supervisor in the script above around line 220
                            ...
                            if (approvalMatrixElement.limitCheckEnabled && parseFloat(txn.total) > parseFloat(employeePurchaseLimit)) {
                          	...
                          	//log.debug("get_approval_users::GET", "email="+employeeEmail);
							
                            if(!employeeEmail){
								log.debug("get_approval_users::GET", "Approval - email is empty");
								return true;
                            }
