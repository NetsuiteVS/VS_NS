...
function lineInit(context,pageinit){
	...
	var lineCount = curr_record.getLineCount({ sublistId: LINE });
	var currIndex = curr_record.getCurrentSublistIndex({ sublistId: LINE });
  	var trans_type = curr_record.getValue({ fieldId: "custbody_ic_trans_type" });
  
    var undepositedFund = curr_record.getValue({fieldId : "custbody_tsa_undeposited_funds"});
	var payment_type = curr_record.getValue({fieldId : "custbody_tsa_bank_acc_pay_type"});	
	
  	var unitType = curr_record.getValue({fieldId : "custbody_unit_type"});
    var rlPartyType = curr_record.getValue({fieldId : "custbody_rp_type"});
  	var unit = curr_record.getText({fieldId : "class"});
    var relatedParty = curr_record.getText({fieldId : "custbody_cseg_tsa_relatedpar"});
	
	var unit_shared_key = curr_record.getValue({fieldId : "custbody_tsa_unit_iu_shared_key"});
	var relparty_shared_key = curr_record.getValue({fieldId : "custbody_tsa_relparty_shared_key"});

	if (unit) unit=unit.replace('\n','');
  	if (relatedParty) relatedParty=relatedParty.replace('\n', '');

    //var r_party_division = curr_record.getText({fieldId : "custbody_rsm_rp_division"});
  	//var unit_division = curr_record.getText({fieldId : "custbody_rsm_uni_division"});
    var r_party_division = curr_record.getValue({fieldId : "custbody_relparty_div_shared_key"});
  	var unit_division = curr_record.getValue({fieldId : "custbody_unit_div_shared_key"});
	...
	window.nlapiSetLineItemDisabled("line","entity", true);
...
*/		
		if(trans_type!=3 && trans_type!=4){ 	
		  ...
		  jQuery('#tbl_line_remove').hide();
			...
			var entity="";
			var entity_type;
			if(trans_type==9) entity_type="vendor"; //fund income
			if(trans_type==10) entity_type="customer"; //fund expense
			if(entity_type){
					var entitySearchObj = Search.create({type: entity_type, filters:[["custentity_tsa_iu_shared_key_entity","anyof",relparty_shared_key]], columns:[Search.createColumn({name: "internalid"})]
				});
				entitySearchObj.run().each(function(result){
				   entity = result.getValue({ name: 'internalid' });
				   return true;
				});
				console.log("lineinit:: entity="+entity);
...
*/					
	...
	//***************** Check            
    function check_shared_key_entity(context){
      ...
      */		
      ...
      //
      // Check Unit shared key linked Customer or Supplier 
      var entity="";
      var entity_type;
      ...
      if(entity_type){
        var entitySearchObj = Search.create({
          type: entity_type, filters:[["custentity_tsa_iu_shared_key_entity","anyof",unit_shared_key]], columns:[Search.createColumn({name: "internalid"})]
        });
        entitySearchObj.run().each(function(result){
          entity = result.getValue({ name: 'internalid' });
          return true;
        });
        console.log("check_shared_key_entity:: entity="+entity);
        if(!entity){
          console.log("check_shared_key_entity - entity alert");
          var alertMessage = translation.get({ collection: 'custcollection__tsa_collection_01', key: 'NO_UNIT_SHKEY_CUST', locale: translation.Locale.CURRENT })();
          if(trans_type==10){var alertMessage = translation.get({ collection: 'custcollection__tsa_collection_01', key: 'NO_UNIT_SHKEY_SUPP', locale: translation.Locale.CURRENT })();} //10=fund expense
          alert(alertMessage);      
        }
      }

      // Check RP shared key linked Cusotmer or Supplier
      var entity="";
      var entity_type;
      if(trans_type==9) entity_type="vendor"; //fund income
      if(trans_type==10) entity_type="customer"; //fund expense
      if(entity_type){
        var entitySearchObj = Search.create({
          type: entity_type, filters:[["custentity_tsa_iu_shared_key_entity","anyof",relparty_shared_key]], columns:[Search.createColumn({name: "internalid"})]
        });
        entitySearchObj.run().each(function(result){
          entity = result.getValue({ name: 'internalid' });
          return true;
        ...
        if(!entity){
          console.log("check_shared_key_entity - entity alert");
          var alertMessage = translation.get({ collection: 'custcollection__tsa_collection_01', key: 'NO_RELP_SHKEY_SUPP', locale: translation.Locale.CURRENT })();
          if(trans_type==10){var alertMessage = translation.get({ collection: 'custcollection__tsa_collection_01', key: 'NO_RELP_SHKEY_CUST', locale: translation.Locale.CURRENT })();}                              
          alert(alertMessage);
        }
      }
      return true;
...
*/		
			...
			switch (context.fieldId)
			{
                ...
                break;
                case "custbody_ic_trans_type":
					console.log("2a postSourcing: context.fieldId="+context.fieldId);
                	check_shared_key_entity(context);
                break;
