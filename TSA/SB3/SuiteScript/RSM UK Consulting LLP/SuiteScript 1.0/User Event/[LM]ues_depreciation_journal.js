...
function afterSubmitLaunchProcess(type) {
	...
	function check_for_asset(jeRecord){	
		...
		for (var line = 1; line <= lineCount; line++) {
			...
			var there_is_an_asset=false;
			var matches = String(memo).match(/FAM\d+/);
			nlapiLogExecution("DEBUG", "","matches="+matches);
			if(matches){
				there_is_an_asset=true;
			}
			else{
			// check account for asset type
                var filters = [new nlobjSearchFilter("custrecord_assettypedeprchargeacc", null, "anyof",	accountId)];
                var assetTypeResults = nlapiSearchRecord("customrecord_ncfar_assettype", null, filters, null);
                if(assetTypeResults){
                    nlapiLogExecution("DEBUG","","AssetType (lib function)="+assetTypeResults[0].id);
                    there_is_an_asset=true;
                }
                var asset_type_id = assetTypeResults && assetTypeResults[0].id;
                nlapiLogExecution("DEBUG", "Asset Type Internal ID", asset_type_id);
			}
		}
	}
