...
define(['N/url', 'N/record', 'N/log', 'N/runtime','N/currentRecord'],
  function(url, record, log, runtime, currentRecord) {
    ...
    function pageInit(scriptContext) {
		...
		if(userObj.roleId!="customrole_tsa_cashier"){ window.nlapiDisableLineItemField("line","account",true); }
		//line_buttons
...
//			$("div").clearQueue();
          ...
          try{
			...
			if( (userObj.id==created_by && (advance_status=="D" || advance_status=="G" || advance_status=="P" || advance_status=="S") ) || ( userObj.roleId=="customrole_tsa_cashier" && (advance_status=="D" || advance_status=="G" || advance_status=="P" || advance_status=="S") ) || !nlapiGetFieldValue("id") ){ //  Workflow locks the records when needed
				window.nlapiDisableLineItemField("line","amount",false); 
				if( userObj.roleId=="customrole_tsa_cashier") window.nlapiDisableLineItemField("line","account",false);
			}
			else{
				if( userObj.roleId=="customrole_tsa_cashier") window.nlapiDisableLineItemField("line","account",false);
			}
